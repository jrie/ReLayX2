...
function relayx (canvasItem, designName, width, height, gridX, gridY, gridStart, gridEnd) {
  ...
  function checkMouseDown (evt) {
    ...
    for (const layoutItem of system.layoutData) {
      ...
      if (dc.isPointInPath(mX, mY)) {
        ...
        hasSelection = true
        if (layoutItem[9] !== -1) { system.activeGroup = layoutItem[9] } else if (mouse.previousSelection !== null && mouse.previousSelection[9] !== -1 && mouse.currentAction === 'grouping') { system.activeGroup = mouse.previousSelection[9] } else { system.activeGroup = null }
        break
      }
    }
    if (hasSelection) {
        ...
        if (mouse.selection[9] === -1) {
          ...
          if (index !== -1) {
            ...
            if (group.length === 1) {
              ...
              for (const layoutItem of system.layoutData) {
                if (layoutItem[9] > groupStart) { --layoutItem[9] } else if (layoutItem[9] === groupStart) { layoutItem[9] = -1 }
              }
              system.activeGroup = null
              system.lastGroupAddition = null
            }
      ...
      if (mouse.previousSelection !== null && mouse.previousSelection[0] === mouse.selection[0]) { mouse.currentAction = 'dragContainer' } else { mouse.currentAction = 'selected' }
      return
    }
    ...
    if (mouse.currentAction === 'resizeCorner') {
      ...
      if (system.mirrorHorizontal) {
        if (containerX >= mouse.threshold) {
          ...
          if (system.spaceGridX > 0) { offsetGridX = offsetGridX = system.spaceGridX } else { offsetGridX = mStartX - system.gridStartX }
          while (offsetX + containerX + offsetGridX <= system.gridEndX - mEndX) {
            offsetX += offsetGridX + containerX
        ...
        if (containerY >= mouse.threshold) {
          if (system.spaceGridY > 0) { offsetGridY = system.spaceGridY } else { offsetGridY = mStartY - system.gridStartY }
          let containerId = createLayoutContainer(mStartX, mStartY, mEndX, mEndY, true)
          system.groups.push([containerId])
          system.activeGroup = system.groups.length - 1
          system.layoutData[system.layoutSize - 1][9] = system.activeGroup
      ...
      for (const layoutItem of system.layoutData) {
        ...
        if (system.drawContainers) {
          if (system.activeGroup !== null && layoutItem[9] === system.activeGroup && mouse.selection[9] === layoutItem[9]) {
            if (mouse.selection[0] === layoutItem[0]) { dc.fillStyle = design.itemSelectionColor[1] } else { dc.fillStyle = design.itemSelectionColor[0] }
  ...
  function drawSelection () {
    ...
    let mEndY = mouse.y
    if (mouse.selection === null) {
      if (mouse.startX < system.gridStartX) { mouse.startX = system.gridStartX } else if (mouse.startX > system.gridEndX) { mouse.startX = system.gridEndX }
      if (mouse.startY < system.gridStartY) { mouse.startY = system.gridStartY } else if (mouse.startY > system.gridEndY) { mouse.startY = system.gridEndY }
      mStartX = mouse.startX
      mStartY = mouse.startY
      mEndX = mouse.x
      mEndY = mouse.y
      if (mEndX > system.gridEndX) { mEndX = system.gridEndX } else if (mEndX < system.gridStartX) { mEndX = system.gridStartX }
      if (mEndY > system.gridEndY) { mEndY = system.gridEndY } else if (mEndY < system.gridStartY) { mEndY = system.gridStartY }
      dc.beginPath()
      if (mouse.snapToGrid) {
        mStartX = Math.floor(mouse.startX / system.gridX) * system.gridX
        mStartY = Math.floor(mouse.startY / system.gridY) * system.gridY
  ...
  function renderNotifications () {
    ...
    let offsetY = 0
    if (system.showHelpNote) { offsetY = window.scrollY + 70 } else { offsetY = window.scrollY + 40 }
  ...
  function handleInputFieldInput (evt) {
    ...
    if (evt.key.length === 1) {
      ...
      if (mouse.cursorAt[2] < text.length) { mouse.selection[10] = mouse.selection[10].substring(0, mouse.cursorAt[2]) + evt.key + mouse.selection[10].substring(mouse.cursorAt[2], text.length) } else { mouse.selection[10] += evt.key }
      ++mouse.cursorAt[2]
  ...
  function loadElectronSlot (evt) {
    ...
    for (const line of storageData) {
      ...
      for (let key = 0; key < storageItemData.length; key++) {
        ...
        if (key === 11) {
          if (storageItemData[11] === '') { storageItemData[11] = null } else {
            const imageMap = new window.Image()
            imageMap.dataset.containerId = storageItemData[0]
            if (storageItemData[11].toLowerCase().indexOf('.gif') === -1) { imageMap.addEventListener('load', addImageToLayout) } else { imageMap.addEventListener('load', addAnimatedImageToLayout) }
            if (storageItemData[11].startsWith('data:')) { imageMap.src = storageItemData[11].replace(/\|\|\|/gm, ',') } else { imageMap.src = storageItemData[11] }
        ...
        for (let key = 0; key < storageItemData.length; key++) {
          ...
          if (key === 11) {
            if (storageItemData[11] === '') { storageItemData[11] = null } else {
              ...
              if (storageItemData[11].startsWith('data:')) { image.src = storageItemData[11].replace(/\|\|\|/gm, ',') } else { image.src = storageItemData[11] }
              if (storageItemData[11] === '') { storageItemData[11] = null } else {
                const imageMap = new window.Image()
                imageMap.dataset.containerId = storageItemData[0]
                if (storageItemData[11].toLowerCase().indexOf('image/gif') === -1) { imageMap.addEventListener('load', addImageToLayout) } else { imageMap.addEventListener('load', addAnimatedImageToLayout) }
                if (storageItemData[11].startsWith('data:')) { imageMap.src = storageItemData[11].replace(/\|\|\|/gm, ',') } else { imageMap.src = storageItemData[11] }
                storageItemData[11] = imageMap
              }
              continue
  ...
  function loadDesign (slot) {
    ...
    if (navigator.userAgent.toLowerCase().indexOf('electron/') === -1) { loadBrowserSlot(slot) } else {
  ...
  function handleKeyboardUp (evt) {
    ...
    if (system.keyMap[evt.keyCode] !== undefined) {
      delete system.keyMap[evt.keyCode]
    }
    if (evt.keyCode === 16) {
    ...
    if (system.shiftPressed) {
      ...
      if (evt.keyCode === 68) {
        ...
        if (system.currentDesign >= system.designNames.length - 1) { system.currentDesign = 0 } else { ++system.currentDesign }
    ...
    if (system.shiftPressed && evt.keyCode === 8) {
      ...
      if (system.drawGrid) { drawGrid() } else { bgContext.clearRect(0, 0, system.gridEndX, system.gridEndY) }
      ...
      if (mouse.currentAction === 'mirrorSelection') {
        if (mouse.selection === null) { mouse.currentAction = 'drawing' } else { mouse.currentAction = 'selected' }
      }
      ...
      if (system.mirrorHorizontal) { system.spaceGridX += system.gridX } else { system.spaceGridY += system.gridY }
    ...
    for (const fileItem of evt.dataTransfer.files) {
      if (fileItem.type === 'image/jpeg' || fileItem.type === 'image/webp' || fileItem.type === 'image/png' || fileItem.type === 'image/gif') {
        if (fileItem.path !== undefined) {
          const imageMap = new window.Image()
          if (fileItem.type !== 'image/gif') { imageMap.addEventListener('load', addImageToLayout) } else { imageMap.addEventListener('load', addAnimatedImageToLayout) }
